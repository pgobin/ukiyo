...
class AvatarManager {
    ...
    fetchFromGravatar(avatarRequest) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            let img = yield this.downloadAvatarImage(avatarRequest.email, 'https://secure.gravatar.com/avatar/' + hash + '?s=54&d=404'), identicon = false;
            if (img === null) {
                img = yield this.downloadAvatarImage(avatarRequest.email, 'https://secure.gravatar.com/avatar/' + hash + '?s=54&d=identicon');
                identicon = true;
            }
            if (img !== null)
    ...
    downloadAvatarImage(email, imageUrl) {
        return __awaiter(this, void 0, void 0, function* () {
            let hash = crypto.createHash('md5').update(email).digest('hex'), imgUrl = url.parse(imageUrl);
