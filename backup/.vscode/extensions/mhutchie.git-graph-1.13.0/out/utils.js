...
function openFile(repo, filePath) {
    return new Promise(resolve => {
        ...
        fs.exists(p, exists => {
            if (exists) {
                vscode.commands.executeCommand('vscode.open', vscode.Uri.file(p), { preview: true, viewColumn: config_1.getConfig().openDiffTabLocation() })
    ...
    return new Promise(resolve => {
        ...
        if (type !== 'U') {
            ...
            if (fromHash === exports.UNCOMMITTED)
                fromHash = 'HEAD';
            vscode.commands.executeCommand('vscode.diff', diffDocProvider_1.encodeDiffDocUri(repo, oldFilePath, fromHash === toHash ? fromHash + '^' : fromHash, type, 'old'), diffDocProvider_1.encodeDiffDocUri(repo, newFilePath, toHash, type, 'new'), title, options)
